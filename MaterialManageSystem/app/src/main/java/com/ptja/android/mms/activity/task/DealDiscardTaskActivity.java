...
import com.ptja.android.mms.bean.DiscardSubmitResultBean;
...
public class DealDiscardTaskActivity extends BaseActivity implements View.OnClickListener {
    ...
    @Bind(R.id.discard_result_layout)
    RelativeLayout discardResultLabel;
    @Bind(R.id.discard_result_value)
    TextView discardResultValue;
    @Bind(R.id.discard_time_value)
    TextView discardTimeValue;
    @Bind(R.id.discard_time_layout)
    RelativeLayout discardTimeLayout;
    @Bind(R.id.addPhoto)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_deal_discard_task);
        ...
        }, "装备报废", null, 0, R.drawable.submit, new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                submit();
            }
    ...
    private void submit() {
        ...
        params.put("result",GlobeVariable.DISCARD_TASK_RESULT.get(discardResultValue.getText().toString()));
        params.put("scrap_content",discardResultValue.getText().toString());
//        params.put("page", currentPageIndex + "");
        params.put("access_token", GlobeVariable.UserInfos.getAccess_token());
        sendPostRequest(params, "提交中", null, UrlConstants.URL_SUBMIT_DISCARD_TASK, new Response.Listener<String>() {
            @Override
            public void onResponse(String s) {
                try {
                    DiscardSubmitResultBean bean = com.alibaba.fastjson.JSONObject.parseObject(s, DiscardSubmitResultBean.class);
                    if (bean.getCode() == 0) {
    ...
    public boolean handleMessage(Message msg) {
        switch (msg.what){
            case 1:
                Toast.makeText(DealDiscardTaskActivity.this, (CharSequence) msg.obj, Toast.LENGTH_SHORT).show();
                break;
            case 2:
        ...
        switch (view.getId()) {
            case R.id.discard_result_layout:
                ...
                showSelectPopWindow(mask, main, names, "选择结果", false, new SelectPicPopupWindow.OnConfirmListener() {
                    @Override
                    public void onConfirm(String result) {
                        discardResultValue.setText(result);
                    }
                });
                break;
            case R.id.discard_time_layout:
                getDate(discardTimeValue);
                break;
        }
    }
