...
public class EquipmentDiscardActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_equip_discard);
        ...
        bean = (EquipmentBean) getIntent().getSerializableExtra("data");
    ...
    private void submit() {
        if (equipNum.getText().toString().equals("")) {
            Toast.makeText(EquipmentDiscardActivity.this, "请输入领取数量", Toast.LENGTH_SHORT).show();
            return;
        }
        try{
            Integer.parseInt(equipNum.getText().toString());
        }catch (NumberFormatException e){
            Toast.makeText(EquipmentDiscardActivity.this, "领取数量只能为数字", Toast.LENGTH_SHORT).show();
            return;
        }
        if (equipPerson.getText().toString().equals("") && equipDept.getText().toString().equals("请选择")) {
            Toast.makeText(EquipmentDiscardActivity.this, "请选择领取单位或领取人", Toast.LENGTH_SHORT).show();
            return;
        }
        if (equipUseType.getText().toString().equals("请选择")) {
            Toast.makeText(EquipmentDiscardActivity.this, "请选择使用类型", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try {
            obj.put("equipment_id",Integer.parseInt(bean.getEquipment_id()));
            obj.put("oper_type",2);
